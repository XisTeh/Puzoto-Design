(function () {
    ...
    if (!isMobile && !prefersReduced) {
        ...
        // Mobile/reduced motion: immediate fake progress
        fill.style.width = '60%';
    ...
    function dismiss() {
        ...
        setTimeout(() => { loader.classList.add('is-loaded'); runEntrance(); }, isMobile ? 100 : 400);
    }
    // Dismiss faster
    setTimeout(dismiss, isMobile ? 50 : 200);

    /* ─── LENIS (desktop only, deferred init) ─────────────────────── */
    let lenis = null;
    function initLenis() {
    ...
    function initWebGL(canvas) {
        ...
        /* PREMIUM CENTRAL 3D: KINETIC HOLOGRAPHIC CORE */
        ...
        // --- 1. PULSING DATA CORE ---
        const coreRadius = 18;
        const coreGeo = new THREE.IcosahedronGeometry(coreRadius, 4);
        ...
        const shell1Mat = new THREE.MeshBasicMaterial({
            color: '#C9D2DA', wireframe: true, transparent: true, opacity: 0.15,
            blending: THREE.AdditiveBlending, depthWrite: false
        });
        ...
        const rings = [];
        for (let i = 0; i < 2; i++) {
            const rGeo = new THREE.TorusGeometry(coreRadius + 14 + (i * 6), 0.1, 4, 100);
    ...
    /* ─── HALO BREATHING (desktop only, opacity/scale only) ────────────── */
    if (!isMobile && !prefersReduced && halo) {
        // Defer to after entrance
        setTimeout(() => {
    ...
    /* ─── SCROLLTRIGGER LOGIC (deferred) ───────── */
    function initScrollAnimations() {
        if (typeof gsap === 'undefined' || typeof ScrollTrigger === 'undefined') return;
        gsap.registerPlugin(ScrollTrigger);

        // Batch ScrollTrigger creation for fewer reflows
    ...
    // Defer ScrollTrigger init to after first meaningful paint
    ...
    /* ─── FLASHLIGHT CARDS & 3D TILT (desktop only) ────── */
    if (!isMobile) {
        // Defer card interactions
        requestAnimationFrame(() => {
